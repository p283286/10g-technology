...
interface ServiceCardProps {
  ...
  isNetworkThreatDetection?: boolean;
}

const ServiceCard = ({ icon, title, description, learnMoreLink, isNetworkThreatDetection }: ServiceCardProps) => {
  const navigate = useNavigate();
  const { language } = useLanguage();
  
  const handleLearnMore = () => {
    if (isNetworkThreatDetection) {
...
const Services = () => {
  ...
  const services = [
    ...
    },
    {
      ...
      title: language === 'zh' ? '電子門鎖安全評估' : 'Electronic Door Lock Security Assessment',
      description: language === 'zh'
        ? '測試電子門鎖系統的安全性，識別潛在漏洞，提供加固方案。'
        : 'Test the security of electronic door lock systems, identify potential vulnerabilities, and provide hardening solutions.'
    }
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((service, index) => (
          <ServiceCard 
            ...
            isNetworkThreatDetection={service.isNetworkThreatDetection}
          />
        ))}
      </div>
      ...
      <div className="mt-16 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="cyber-card p-6 bg-cyber-gradient-light">
            ...
            <Button variant="secondary">{language === 'zh' ? '了解詳情' : 'Learn More'}</Button>
          </div>
